...
Tests the reactive primitives and routing helpers from `@blueshed/railroad`. Pure in-memory, no I/O.
...
`@blueshed/railroad` is infrastructure — `signals.test.ts` tests the generic reactive layer, not your app.
